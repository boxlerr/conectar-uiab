...
const AltaSchema = z.object({
  ...
  email_compras: z.string().trim().toLowerCase().email("El correo de compras no es válido").max(160).optional().or(z.literal("")),
  email_mantenimiento: z.string().trim().toLowerCase().email("El correo de mantenimiento no es válido").max(160).optional().or(z.literal("")),
    ...
    if (!empresaIdFinal) {
      const { data: emp, error: empErr } = await db
        ...
        .insert({
          ...
          estado: "aprobada",
          email,
          email_compras: alta.email_compras ?? null,
          email_mantenimiento: alta.email_mantenimiento ?? null,
          telefono: alta.telefono ?? null,
          sitio_web: alta.sitio_web ?? null,
          localidad: alta.localidad ?? null,
          direccion: alta.direccion ?? null,
